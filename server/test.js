...
(async () => {
  ...
  try {
    // Your code here
    const newDog = await Puppy.build({name: "Trudy", age_yrs: 2, weight_lbs: 38, breed: "Brittany Spaniel", microchipped: false }); 
    await newDog.save()
  ...
  try {
    // Your code here
    const newDog = await Puppy.create({ name: 'Beans', age_yrs: 1.6, weight_lbs: 42, breed: 'Bulldog', microchipped: true});
  } catch (err) {
    console.error(err)
  }
